...
/**
 * 目的:用户只需要传递参数,工具API就可以发起请求,将返回的结果交给用户
 * @param url
 * @param params K V
 * @param charset 考虑用户使用格式
 * @return
 */
	public String doGet(String url,Map<String,String> params,String charset){
		...
		if(params != null){
			url+="?";//拼接?号
			for (Map.Entry<String, String> entry : params.entrySet()) {//遍历map集合  Map<entry<k,v>>实际遍历map 是.entrySet()  
				String key=entry.getKey();
				String value=entry.getValue();
				url+=key+"="+value+"&";
			}
			url=url.substring(0, url.length()-1);//最后多一个&,截取0到length-1位置
		}
