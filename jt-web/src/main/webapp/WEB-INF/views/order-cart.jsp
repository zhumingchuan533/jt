...
<!--shortcut end-->
	<form id="orderForm" class="hide">
		...
		<c:forEach items="${carts}" var="cart" varStatus="status">
			...
			<input type="hidden" name="orderItems[${status.index}].itemId" value="${cart.itemId}"/><!--${status.index}下标  -->
		...
		<input type="hidden" name="orderShipping.receiverName" value="祝鸣川"/>
		...
		<input type="hidden" name="orderShipping.receiverCity" value="深圳市"/>
		<input type="hidden" name="orderShipping.receiverDistrict" value="宝安区"/>
		<input type="hidden" name="orderShipping.receiverAddress" value="西乡商务大厦"/>
	</form>
